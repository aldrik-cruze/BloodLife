            ...
            tr.innerHTML = `
                <td data-label="Name">
                    <div style="display:flex;align-items:center;gap:.6rem;">
                        <div style="width:36px;height:36px;border-radius:50%;background:linear-gradient(135deg,#667eea,#764ba2);display:flex;align-items:center;justify-content:center;color:white;font-weight:700;flex-shrink:0;">
                            ${initials}
                        </div>
                        <div>
                            <div style="font-weight:600;color:var(--admin-text);">${name}</div>
                            <div style="font-size:.8rem;color:var(--admin-text-muted);">${donor.email}</div>
                        </div>
                    </div>
                </td>
                <td data-label="Blood">
                    <span style="display:inline-block;padding:.4rem .8rem;background:linear-gradient(135deg,#ff4757,#ff6b81);color:white;border-radius:8px;font-weight:700;">
                        ${bloodGroup}
                    </span>
                </td>
                <td data-label="Phone" style="color:var(--admin-text);">📞 ${donor.phone}</td>
                <td data-label="Address" style="color:var(--admin-text-muted);">📍 ${donor.address}</td>
    ...
    try {
        ...
        requests.forEach(req => {
            ...
            tr.innerHTML = `
                ...
                <td data-label="Units" style="font-weight:600;color:var(--admin-text);">${req.unit} ${req.unit > 1 ? 'units' : 'unit'}</td>
                <td data-label="Hospital" style="color:var(--admin-text);">🏥 ${req.hospital}</td>
                <td data-label="Date" style="color:var(--admin-text-muted);">📅 ${req.neededDate}</td>
                <td data-label="Contact" style="color:var(--admin-text);">📞 ${req.phone}</td>
                <td>
